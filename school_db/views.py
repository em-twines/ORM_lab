...
def problem_one(request):
  ...
  students_gt_three = Student.objects.filter(gpa__gt = 3.0).order_by("-gpa")
  for student in students_gt_three:
...
def problem_two(request):
  ...
  instructors_chosen = Instructor.objects.filter(hire_date__lt = '2010-01-01').order_by("hire_date")
  for instructor in instructors_chosen:
    print(f"Full Name: {instructor.first_name} {instructor.last_name}: hired on: {instructor.hire_date}")
  return complete(request)
...
def problem_three(request):
  ...
  print(f"{Instructor.objects.get(pk = 2).first_name} {Instructor.objects.get(pk = 2).last_name}:") 
...
def problem_six(request):
    ...
    student_id = 11
    student_to_update = Student.objects.filter(id = 11).update(gpa = 4.0)
    print(f"ID: {Student.objects.get(id = 11).id} \nFull Name: {Student.objects.get(id = 11).first_name} {Student.objects.get(id = 11).last_name} \nGPA: {Student.objects.get(id = 11).gpa}")
    return complete(request)
...
def bonus_problem(request):
  ...
  for inst in instructor_count:
    print(f"Instructor: {inst.last_name} {inst.first_name} Courses: {inst.id_num}")

  # instructors_to_print = Instructor.objects.pk =
    
  # courses = Course.objects.order_by().values("instructor_id").distinct()
  # print(courses)
  ...
  # courses = Course.objects.values("instructor_id")
  # print(courses)
  # num = 0
  # while num >= courses:
  #   courses_agg += courses[num] 
  #   print(courses_agg)
  #   num += 1
  
  # course = Course.objects.values("id")
  # final = Instructor.objects.filter(course).distinct()
  # # for instructor in instructors:
  # print(final)
  #   print(instructor.last_name)

  # print(f"{Instructor.objects.filter(instructors.first_name)}, {Instructor.objects.filter(instructors.last_name)}")
  return complete(request)
